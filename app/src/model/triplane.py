...
class TriplaneGenerator(nn.Module):
    ...
    def __init__(
        ...
        self.refine = nn.Sequential(
            nn.Conv2d(plane_ch, plane_ch, 3, padding=1), nn.GELU(),
            nn.Conv2d(plane_ch, plane_ch, 3, padding=1), nn.GELU(),
    ...
    def forward(self, img_feat: torch.Tensor) -> torch.Tensor:
        ...
        planes = flat.view(B * 3, self.plane_ch,
                           self.plane_res, self.plane_res) # [B*3, C, H, W]
        planes = self.refine(planes)
        return planes.view(B, 3, self.plane_ch,
                           self.plane_res, self.plane_res) # [B, 3, C, H, W]

def _bilinear_sample(plane: torch.Tensor, coords: torch.Tensor) -> torch.Tensor:
    """
    MPS-compatible manual bilinear sampling.  Replaces F.grid_sample whose
    backward is not yet implemented on MPS (aten::grid_sampler_2d_backward).
...
def volume_render(
    ...
    """
    Classic NeRF volume rendering.  Pure torch — runs on MPS.
...
class TriplaneNeRF(nn.Module):
    ...
    def render_full(
        ...
        """
        Render a complete H×W image in chunks (avoids OOM on MPS).
...
def make_optimizer(model: TriplaneNeRF, lr_gen: float = 5e-4, lr_nerf: float = 5e-4,
