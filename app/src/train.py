...
MPS notes
---------
  - All tensor ops run on MPS.
  - DataLoader num_workers=0 (fork-based multiprocessing breaks on macOS MPS).
  - Gradient checkpointing is not used — 24 GB unified memory is enough.
  - Render chunk size is tunable if you hit OOM during full-image render.
"""
...
class RenderViewLoader:
    ...
    def sample(self, cat: str, obj_id: str, k: int = 4,
...
class Visualiser:
    ...
    def update(
        ...
        ax.plot(self._steps, self._photo, color="#E8672A", lw=1.0, label="photo", alpha=0.85)
        ...
        if self._val_steps:
            ax.scatter(self._val_steps, self._val_loss,
                       color="#FF6B9D", s=40, zorder=5, label="val")
        ax.set_title(f"Loss  (epoch {epoch}, step {step})", color="white", fontsize=9)
        ax.set_xlabel("step", color="#aaa", fontsize=8)
        ax.tick_params(colors="#aaa", labelsize=7)
        for sp in ax.spines.values():
            sp.set_color("#444")
        ax.legend(fontsize=7, facecolor="#2a2a2a", labelcolor="white", loc="upper right")
        ...
        # ── [4] GT point cloud (X-Z top-down) ────────────────────────────────
        p = gt_pts.cpu().numpy()
        axes[4].scatter(p[:, 0], p[:, 2], s=1.5, c="#20B2AA", alpha=0.5)
        axes[4].set_title("GT point cloud (X-Z)", color="white", fontsize=9)
        axes[4].set_aspect("equal")
        axes[4].tick_params(colors="#aaa", labelsize=6)
        for sp in axes[4].spines.values():
            sp.set_color("#444")
        axes[4].grid(True, linewidth=0.3, alpha=0.3, color="#444")
        ...
        _path = save_path or (
            ...
            if self.vis_dir else None
        )
        if _path:
            fig.savefig(_path, dpi=100, bbox_inches="tight",
...
def load_checkpoint(model, path, optimizer=None, device=None):
    ...
    print(f"  [ckpt] loaded epoch={ckpt['epoch']} step={ckpt['step']} "
...
def train(
    ...
    val_ds   = OmniObject3DDataset(omni_root, split="val",   categories=cats,
                                   random_view=False)

    # num_workers=0: MPS + macOS fork-based multiprocessing don't mix
    ...
    # ── model + optimizer ─────────────────────────────────────────────────────
    model = TriplaneNeRF(
        plane_ch=32, plane_res=64, nerf_hidden=128, n_samples=64,
        near=0.5, far=4.5,
    ).to(device)
    ...
    print(f"Model params: total={total_p:,}  trainable={trainable:,}  "
    ...
    from .model.triplane import plane_tv_loss, plane_l2_loss, volume_render
    from .model.triplane import sample_along_rays
    ...
    for epoch in range(start_epoch, n_epochs + 1):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            for b in range(B):
                ...
                for vi, view in enumerate(views):
                    ...
                    view_loss   = (F.mse_loss(rgb_map, rgb_gt_rays)
                                   + 0.1 * F.l1_loss(rgb_map, rgb_gt_rays))
                    ...
                    if b == 0 and vi == 0 and vis is not None:
                        with torch.no_grad():
                            ...
                            # quality, fast enough to run every step on MPS.
                            # Reuse already-computed planes (no second encoder pass).
                            _VIS_H, _VIS_W = 64, 64
                            rendered = model.render_full(
                                img[0:1], c2w, focal,
                                H=_VIS_H, W=_VIS_W, chunk=_VIS_H * _VIS_W,
                                planes=plane_b.detach(),
                            )
                            _vis_pred_render = rendered["rgb"].permute(2, 0, 1).clamp(0, 1)
                            _vis_gt_render = F.interpolate(
                                rgb_gt.unsqueeze(0), size=(_VIS_H, _VIS_W),
            ...
            if vis is not None:
                vis.record(
                    global_step,
                    loss.item(), photo_loss.item(),
                    tv_loss.item(), l2_loss.item(),
                )
                vis.update(
                    step        = global_step,
                    epoch       = epoch,
        ...
        print(f"Epoch {epoch:3d} done | train_loss={train_avg:.5f} | "
...
def visualise_reconstruction(
    ...
    input_img = dino_tfm(
        Image.open(root / "raw" / cat / obj_id / raw_frame["file_path"])
              .convert("RGB")
