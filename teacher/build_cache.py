...
def prepare_image(img_path: Path, mask_path: Path, size: int = 512, fg_ratio: float = 0.85) -> Image.Image | None:
    ...
    return Image.fromarray((final * 255).astype(np.uint8)).resize((size, size), Image.BICUBIC)


# ── point cloud sampling ──────────────────────────────────────────────────────

def sample_points(mesh: trimesh.Trimesh, n: int = N_POINTS, seed: int = 0) -> np.ndarray:
...
def main() -> None:
    ...
    p.add_argument("--limit",         type=int,  default=None, help="Process at most N entries (for testing)")
    args = p.parse_args()
    ...
    for idx, e in enumerate(entries):
        ...
        try:
            ...
            with torch.no_grad():
                ...
                meshes = model.extract_mesh(codes, has_vertex_color=True, resolution=args.mc_resolution)
            src_mesh = meshes[0]

            # ── 3. ICP align to Pix3D GT ──────────────────────────────────
            tgt_mesh = trimesh.load(args.pix3d_root / e["model"], process=False, force="mesh")
            ...
            canon_mesh  = trimesh.Trimesh(vertices=canon_verts, faces=src_mesh.faces, process=False)
            ...
            stats["ok"] += 1
            print(f"[{idx:5d}] {stem}: cost={cost:.4f}, {int(len(pts))}pts, {elapsed:.1f}s")
