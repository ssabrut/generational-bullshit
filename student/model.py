...
class VertexPatchCrossAttn(nn.Module):
    def __init__(
        self, d_vert: int, d_patch: int = DINO_DIM, d_attn: int = 128, n_heads: int = 4
    ):
        super().__init__()
    ...
    def forward(self, h_v: torch.Tensor, patches: torch.Tensor) -> torch.Tensor:
        ...
        P = patches.shape[1]
        q = (
            self.q(h_v).view(B, V, self.n_heads, self.d_head).transpose(1, 2)
        )  # (B,H,V,Dh)
        k = (
            self.k(patches).view(B, P, self.n_heads, self.d_head).transpose(1, 2)
        )  # (B,H,P,Dh)
...
class GCNBlock(nn.Module):
    ...
    def forward(self, h_v: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        ...
        deg = (
            torch.zeros(V, device=h_v.device, dtype=h_v.dtype)
            .index_add(0, dst, torch.ones_like(dst, dtype=h_v.dtype))
            .clamp(min=1)
            .view(1, V, 1)
        )
        agg = agg / deg
