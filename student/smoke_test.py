...
def main() -> None:
    torch.manual_seed(0)
    device = "cpu"  # MPS is hit-or-miss with DinoV2 ops; CPU is safer for the smoke test

    print(f"Loading dataset…  device={device}")
    ds = TeacherCacheDataset(cache_root=ROOT / "data/teacher_cache", categories=("chair",))
    print(f"  {len(ds)} entries available")
    sample = ds[0]
    image   = sample["image"].unsqueeze(0).to(device)          # (1, 3, 224, 224)
    target  = sample["points"].unsqueeze(0).to(device)         # (1, N, 3) — unit-sphere-normalized
    ...
    print(f"  template: {tpl.verts.shape[0]} verts, {tpl.faces.shape[0]} faces, "
    ...
    print(f"  built in {time.time() - t0:.1f}s | trainable {n_trainable / 1e6:.2f}M / "
          f"total {n_total / 1e6:.2f}M params")

    # Capture the initial (un-deformed) prediction
    with torch.no_grad():
        out0 = model(image)
        loss0, comp0 = compute_loss(out0["verts"], target, model.template_edges, laplacian)
    ...
    render_points(target_np, axes[1]);  axes[1].set_title("target points")
    render_mesh(tpl_norm,  faces, axes[2]); axes[2].set_title("template (before)")
    render_mesh(pred_verts, faces, axes[3]); axes[3].set_title(f"deformed (after {n_steps} steps)")

    fig.delaxes(axes[4])
    ax_loss = fig.add_subplot(1, 5, 5)
    ax_loss.plot(losses, color="#aa3355")
    ax_loss.set_xlabel("step"); ax_loss.set_ylabel("total loss")
