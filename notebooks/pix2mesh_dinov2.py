...
print(
    f"GCN  in: {tuple(dummy_verts.shape)}  out: {tuple(new_v.shape)}  hidden: {tuple(h_out.shape)}"
)
...
model = Furniture3D(encoder=encoder, hidden_dim=512, n_stages=3, n_gcn_blocks=5).to(
    DEVICE
)
...
def _render_step(
    ...
    if val_losses:
        x_val = [
            int(i * len(step_losses) / len(val_losses)) for i in range(len(val_losses))
        ]
        ax_loss.plot(
            x_val,
            val_losses,
    ...
    for i, (verts, faces) in enumerate(zip(pred_verts_list, faces_list)):
        ...
        poly = Poly3DCollection(
            tris, alpha=0.3, facecolor=stage_colors[i], edgecolor="#555", linewidth=0.1
        )
...
def train(
    ...
    for epoch in epoch_bar:
        ...
        step_bar = tqdm(
            loader, desc=f"Train {epoch}/{n_epochs}", leave=False, unit="batch"
        )
        for batch in step_bar:
            ...
            real_logits = disc(
                gt[:, : pred_verts_final.shape[1], :].to(device), adj_final
            )
...
# %%
smoke_model = Furniture3D(
    encoder=encoder, hidden_dim=512, n_stages=3, n_gcn_blocks=5
).to(DEVICE)
smoke_disc = MeshDiscriminator(hidden_dim=256).to(DEVICE)

fpn_params = list(smoke_model.encoder.lateral.parameters()) + list(
    smoke_model.encoder.output_convs.parameters()
)
...
for step in range(3):
    ...
    trans_s = torch.zeros(bs, 3, device=DEVICE)
    trans_s[:, 2] = 2.0
    ...
    print(
        f"  Step {step+1} | G={loss_g.item():.4f} | Adv={loss_adv.item():.4f} | D={loss_d.item():.4f}"
    )
...
model = Furniture3D(encoder=encoder, hidden_dim=512, n_stages=4, n_gcn_blocks=5).to(
    DEVICE
)
discriminator = MeshDiscriminator(hidden_dim=256).to(DEVICE)
...
optimizer_disc = torch.optim.AdamW(
    discriminator.parameters(), lr=3e-4, weight_decay=1e-4
)
...
def plot_history(history):
    ...
    for ax, yscale in zip(axes, ["linear", "log"]):
        ax.plot(history["train"], label="Train", c="#4A90D9", lw=2)
        if has_val:
            ax.plot(history["val"], label="Val", c="#E8672A", lw=2)
        ax.set_xlabel("Epoch")
        ax.set_ylabel("Loss")
...
def visualise(pred_verts_list, gt_pts, faces_list, title=""):
    ...
    gp = gt_pts[0].cpu().numpy()
    ax.scatter(*gp.T, s=1, c="#20B2AA", alpha=0.4)
    ax.set_title("GT Point Cloud", fontsize=9)
    ax.set_axis_off()
...
visualise(
    out["vertices"],
    batch["gt_points"].to(DEVICE),
    out["faces"],
    title="DINOv2+FPN — validation sample",
)
...
save_ckpt(
    ...
    optimizer_disc,
    epoch=epochs,
    path="./furniture3d_dinov2_gan.pth",
)
