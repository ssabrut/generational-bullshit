...
def train(model, disc, loader, optimizer, optimizer_disc, scheduler=None, n_epochs=100, val_loader=None,
          vis_every=20, w_adv=0.1, device=DEVICE, ckpt_dir="./checkpoints", resume_ckpt=None):
    ...
    epoch_bar = tqdm(range(start_epoch, n_epochs + 1), desc="Epochs", unit="epoch")
...
RESUME_CKPT = None  # set to e.g. "./checkpoints/ckpt_epoch_0005.pth" to resume

history = train(model, discriminator, train_loader, optimizer, optimizer_disc,
                scheduler=scheduler, n_epochs=epochs, w_adv=0.15, vis_every=5,
                val_loader=val_loader, ckpt_dir=CKPT_DIR, resume_ckpt=RESUME_CKPT)
...
# %%
def save_quantized(model, disc, path_model="./furniture3d_dinov2_quantized.pth",
                   path_disc="./discriminator_quantized.pth"):
    model.eval()
    disc.eval()
    ...
    q_disc = torch.ao.quantization.quantize_dynamic(
        disc.cpu(), {nn.Linear}, dtype=torch.qint8
    )
...
save_quantized(model, discriminator)
